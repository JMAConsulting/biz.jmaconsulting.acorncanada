...
class CRM_Mailing_BAO_Mailing extends CRM_Mailing_DAO_Mailing {
  ...
  public static function getRecipients($mailingID) {
    ...
    CRM_Core_DAO::executeQuery("SET SESSION optimizer_switch='block_nested_loop=off'");
    ...
    foreach ($recipientsGroup as $groupType => $groupIDs) {
      ...
      while ($groupDAO->fetch()) {
        ...
        if ($groupDAO->saved_search_id && ($groupDAO->cache_date == NULL || $groupDAO->is_hidden)) {
          CRM_Contact_BAO_GroupContactCache::load($groupDAO);
        }
        if ($groupType == 'Include') {
          $includeSmartGroupIDs[$groupDAO->id] = $groupDAO->id;
        }
        elseif ($groupType == 'Exclude') {
          $excludeSmartGroupIDs[$groupDAO->id] = $groupDAO->id;
        }
    ...
    CRM_Core_DAO::executeQuery("SET SESSION optimizer_switch='block_nested_loop=on'");
  }
